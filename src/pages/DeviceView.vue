...
export default {
  ...
  mounted() {
    this.loadApplicationDetails().then(res => {
  ...
  methods: {
    ...
    async loadApplicationDetails() {
      ...
      this.deviceType = await this.$store.state.main.sdk.models.deviceType.get(this.device.is_of__device_type.__id)
      this.releaseInfo = await this.$store.state.main.sdk.models.release.get(this.device.is_running__release.__id)
      console.log(this.device.current_services)
    },
